...
After any channel configuration change (including Slack setup), the gateway needs to restart. If the setup form was used, this happens automatically. However, if you configured a channel through other means (e.g., via a tool or API), output this command to trigger a gateway restart:

```
PAZI_COMMAND:RESTART_GATEWAY
```

The dashboard will show a loading overlay while the gateway restarts and confirm when complete.
...
- **Typing indicator**: While the agent is processing a reply, a typing/thinking animation is shown in Slack (via Slack's native streaming or a reaction-based indicator). This gives users visual feedback that the agent is working on a response.
...
- Show a thinking animation while processing replies
...
- The command marker must appear exactly as shown — `PAZI_COMMAND:SLACK_SETUP` or `PAZI_COMMAND:RESTART_GATEWAY`
...
- The Slack setup form handles the full flow including gateway restart, so you do not need to send `PAZI_COMMAND:RESTART_GATEWAY` separately after the Slack form
