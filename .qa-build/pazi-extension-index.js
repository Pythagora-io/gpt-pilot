...
function createSetGoalTool(deps) {
	return {
		name: "set_goal",
		label: "Set Goal",
		description: "Propose a goal for the user. Opens a confirmation card in the user's dashboard showing the goal details. The user can confirm or reject. Use this when the user asks you to set, create, or track a goal. Returns the created goal ID on confirmation.",
		parameters: Type.Object({
			...
			timeoutMs: Type.Optional(Type.Number({ description: "Max wait time in ms (default: 300000)" })),
		...
		async execute(_toolCallId, params, signal) {
			try {
				...
				const timeoutMs = typeof params.timeoutMs === "number" && params.timeoutMs > 0 ? params.timeoutMs : 3e5;
				...
				if (!created.ok) return json$2({ error: created.error });
				...
				while (true) {
					if (signal?.aborted) return json$2({
						status: "aborted",
						requestId
					});
					...
					if (status === "completed") {
						const goalId = result.data.request.result;
						return json$2({
							status: "completed",
							requestId,
					...
					if (status === "cancelled") return json$2({
						status: "cancelled",
						requestId,
						message: `Goal "${title}" was rejected by the user.`
					});
					if (status === "expired") return json$2({
						status: "expired",
						requestId
					});
					if (Date.now() >= deadline) return json$2({
						status: "timeout",
						requestId
					});
					const waitMs = Math.min(pollIntervalMs, deadline - Date.now());
					if (waitMs > 0) {
						if (await sleep$1(waitMs, signal) === "aborted") return json$2({
							status: "aborted",
							requestId
						});
					}
